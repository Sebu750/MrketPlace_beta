...
export default function DesignerCollectionForm() {
  ...
  const [lookbookFiles, setLookbookFiles] = useState(Array(6).fill(null));
  const [lookbookPreviews, setLookbookPreviews] = useState(Array(6).fill(null));
  const coverRef = useRef(null);
  const lookbookRefs = useRef([]);
  ...
  const handleLookbookSelect = (idx, file) => {
    if (!file || !file.type.startsWith("image/")) return;
    const newFiles = [...lookbookFiles];
    const newPreviews = [...lookbookPreviews];
    if (newPreviews[idx]) URL.revokeObjectURL(newPreviews[idx]);
    newFiles[idx] = file;
    newPreviews[idx] = URL.createObjectURL(file);
    setLookbookFiles(newFiles);
    setLookbookPreviews(newPreviews);
  };

  const removeLookbook = (idx, e) => {
    e.stopPropagation();
    const newFiles = [...lookbookFiles];
    const newPreviews = [...lookbookPreviews];
    if (newPreviews[idx]) URL.revokeObjectURL(newPreviews[idx]);
    ...
    setLookbookFiles(newFiles);
    setLookbookPreviews(newPreviews);
    if (lookbookRefs.current[idx]) lookbookRefs.current[idx].value = "";
  };

  const handleSave = async (action) => {
    setUploadError("");
    if (!form.name || !form.season || !form.category || !form.description) {
    ...
    try {
      ...
      if (coverFile) {
        ...
        const { data: coverRes } = await API.post("/upload/image", coverFd, {
      ...
      const lbFiles = lookbookFiles.filter(Boolean);
      if (lbFiles.length > 0) {
        const lbFd = new FormData();
        lbFiles.forEach((f) => lbFd.append("lookbook", f));
        const { data: lbRes } = await API.post("/upload/collection", lbFd, {
          headers: { "Content-Type": "multipart/form-data" },
        });
        lookbookUrls = (lbRes.data || []).map((d) => d.url);
      }
      ...
      const payload = {
        ...
        craftTraditions: form.crafts,
        coverImage: coverImageUrl,
        lookbookImages: lookbookUrls,
      };
      ...
      setUploadError(err?.message || err || "Failed to save collection");
    } finally {
      setUploading(false);
    }
  };
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        ...
        <div className="flex items-center gap-3">
          <button onClick={() => handleSave("draft")} disabled={uploading}
            className="px-5 py-2.5 text-[10px] uppercase tracking-[0.18em] text-charcoal-600 border border-stone-200 hover:border-charcoal-300 transition-colors disabled:opacity-50">
        ...
        <div className="bg-white border border-stone-200 p-8 space-y-6 max-w-3xl">
          ...
          <Field label="Cover Image" required hint="The primary image representing this collection">
            ...
            <div onClick={() => coverRef.current?.click()}
              className="border-2 border-dashed border-stone-300 bg-stone-50 p-16 text-center hover:border-bronze-300 transition-colors cursor-pointer relative group">
              {coverPreview ? (
                <>
                  ...
                  <button onClick={removeCover}
                    className="absolute top-2 right-2 w-7 h-7 bg-charcoal-900/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <div key={i} className="relative">
                  <input
                    ref={(el) => { lookbookRefs.current[i] = el; }}
                    type="file" accept="image/jpeg,image/png,image/webp" className="hidden"
                    onChange={(e) => handleLookbookSelect(i, e.target.files?.[0])} />
                  <div onClick={() => lookbookRefs.current[i]?.click()}
                    className="aspect-[3/4] border-2 border-dashed border-stone-200 bg-stone-50 flex flex-col items-center justify-center hover:border-bronze-300 transition-colors cursor-pointer group relative overflow-hidden">
                    {lookbookPreviews[i] ? (
                      <>
                        <img src={lookbookPreviews[i]} alt={`Lookbook ${i + 1}`} className="absolute inset-0 w-full h-full object-cover" />
                        <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                          <span className="text-white text-[9px] uppercase tracking-wider">Change</span>
                        </div>
                        <button onClick={(e) => removeLookbook(i, e)}
                          className="absolute top-1.5 right-1.5 w-6 h-6 bg-charcoal-900/80 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
          ...
          {uploadError && (
            <div className="bg-red-50 border border-red-200 text-red-600 text-sm px-4 py-3">{uploadError}</div>
          )}
        ...
        <div className="space-y-6">
          <div className="bg-white border border-stone-200 p-8 max-w-2xl">
            ...
            <div className="bg-stone-50 border border-stone-200 overflow-hidden">
              <div className="aspect-[16/7] bg-stone-200 flex items-center justify-center relative overflow-hidden">
                {coverPreview ? (
                  <img src={coverPreview} alt="Cover" className="absolute inset-0 w-full h-full object-cover" />
                ) : (
