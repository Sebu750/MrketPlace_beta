    ...
    <div className="bg-white min-h-screen">
      ...
      <div className="sticky top-16 z-40 bg-white/95 backdrop-blur-md border-b border-bronze-200/50">
        <div className="max-w-7xl mx-auto px-6 py-4 flex flex-wrap items-center gap-4">
          ...
          {/* Filter toggle (mobile) — opens drawer */}
          <button onClick={() => setSidebarOpen(true)} className="md:hidden px-4 py-2.5 bg-white border border-stone-200 text-charcoal-400 text-sm">
            Filters {hasActive && <span className="ml-1 inline-flex w-4 h-4 bg-bronze-400 text-white text-[9px] rounded-full items-center justify-center">{Object.values(filters).filter(v => v !== "All").length + (searchQuery ? 1 : 0)}</span>}
          </button>
      ...
      <div className="max-w-7xl mx-auto px-6 py-10">
        <div className="flex gap-10">
          ...
          {/* ═══════════════════════════════════════════════════════════
              3. FILTER SIDEBAR (desktop: inline, mobile: drawer)
          ═══════════════════════════════════════════════════════════ */}
          {/* Mobile filter drawer overlay */}
          {sidebarOpen && (
            <div className="fixed inset-0 z-50 md:hidden" onClick={() => setSidebarOpen(false)}>
              <div className="absolute inset-0 bg-charcoal-950/40 backdrop-blur-sm" />
              <aside className="absolute left-0 top-0 bottom-0 w-80 max-w-[85vw] bg-white overflow-y-auto animate-slideInLeft" onClick={(e) => e.stopPropagation()}>
                <div className="flex items-center justify-between px-6 py-5 border-b border-stone-200">
                  <span className="text-xs uppercase tracking-[0.2em] text-charcoal-900">Filters</span>
                  <button onClick={() => setSidebarOpen(false)} className="text-charcoal-400 hover:text-charcoal-900">
                    <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}><path strokeLinecap="round" d="M18 6 6 18M6 6l12 12" /></svg>
                  </button>
                </div>
                <div className="px-6 py-6 space-y-8">
                  {Object.entries(filterConfig).map(([key, options]) => (
                    <div key={key}>
                      ...
                      <div className="flex flex-col gap-1.5">
                        {options.map((opt) => (
                          <button key={opt} onClick={() => updateFilter(key, opt)}
                            className={`text-left px-3 py-2 text-xs tracking-wide transition-all duration-200 ${filters[key] === opt ? "bg-charcoal-900 text-white" : "text-charcoal-400 hover:text-charcoal-900 hover:bg-stone-50"}`}>
                            {opt}
                          </button>
                        ))}
                      </div>
                  ...
                  {hasActive && (
                    <button onClick={() => { resetFilters(); setSidebarOpen(false); }} className="w-full py-3 border border-stone-200 text-xs uppercase tracking-wider text-charcoal-500 hover:border-charcoal-900 transition-colors">
                      Clear All Filters
                    </button>
                  )}
                </div>
                ...
                {products.map((p) => (
                  <Link key={p._id || p.id} to={`/pieces/${p.slug || p._id || p.id}`} className="group block">
                    <div className="relative aspect-[3/4] overflow-hidden bg-stone-50">
                      <img src={p.images?.[0] || p.img || "/assets/images/placeholder.webp"} alt={p.name} loading="lazy" decoding="async"
