...
export default function AdminPanel() {
  ...
  return (
    <div className="min-h-screen bg-white">
      {/* Header */}
      <header className="border-b border-noir-100 bg-stone-50">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-10">
          ...
          <main className="lg:col-span-10">
            ...
            <div className="mb-8 flex items-center justify-between">
              <div>
                <h1 className="font-serif text-3xl text-noir-900">Platform Overview</h1>
                <p className="mt-1 text-sm text-noir-500">Monitor and manage all aspects of Adorzia marketplace.</p>
              </div>
              <div className="text-right">
                <p className="text-xs text-noir-400">Last updated</p>
                <p className="text-sm text-noir-900">Jun 8, 2026 · 10:34 AM</p>
              </div>
            </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
              ...
              <div className="border border-noir-100">
                <div className="px-6 py-4 border-b border-noir-100 flex items-center justify-between">
                  <h2 className="text-sm font-medium text-noir-900">Pending Approvals</h2>
                ...
                <div className="divide-y divide-noir-100">
                  {pendingApprovals.map((item, i) => (
                    <div key={i} className="px-6 py-4">
                      <p className="text-sm text-noir-900">{item.name}</p>
                      <p className="text-xs text-noir-400 mt-0.5">
                        {item.type} · {item.city || item.designer} · {item.date}
                      </p>
                      <div className="mt-2 flex gap-2">
                        <button className="text-xs bg-noir-900 text-white px-3 py-1 hover:bg-noir-800 transition-colors">
                          Approve
                        </button>
                        <button className="text-xs border border-noir-200 text-noir-600 px-3 py-1 hover:border-noir-400 transition-colors">
                          Review
                        </button>
                      </div>
                    </div>
              ...
              {/* Recent Activity */}
              <div className="border border-noir-100">
                <div className="px-6 py-4 border-b border-noir-100">
                  <h2 className="text-sm font-medium text-noir-900">Recent Activity</h2>
                </div>
                <div className="divide-y divide-noir-100">
                  {recentActivity.map((activity, i) => (
                    <div key={i} className="px-6 py-4">
                      <p className="text-sm text-noir-900">{activity.action}</p>
                      <p className="text-xs text-noir-400 mt-0.5">{activity.detail}</p>
                      <p className="text-xs text-noir-300 mt-1">{activity.time}</p>
                    </div>
                  ))}
                </div>
              </div>
            ...
            <div className="mt-8 grid grid-cols-1 md:grid-cols-4 gap-4">
              {[
                ...
                <button key={i} className="border border-noir-100 p-5 hover:border-noir-300 transition-colors text-left group">
                  <p className="text-xs uppercase tracking-wider text-noir-400 mb-2">{action.label}</p>
                  <p className="font-serif text-base text-noir-900 group-hover:text-gold-500 transition-colors">{action.action}</p>
                  <p className="text-xs text-noir-500 mt-1">{action.desc}</p>
                </button>
              ))}
            </div>

            {/* System status */}
            <div className="mt-8 border border-noir-100 p-6 bg-stone-50">
              <div className="flex items-center justify-between">
                <div>
                  <p className="text-xs uppercase tracking-wider text-noir-400 mb-1">System Status</p>
                  <p className="text-sm text-noir-900">All systems operational</p>
                </div>
