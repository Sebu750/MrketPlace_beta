    ...
    <div className="group relative">
      ...
      <Link to={`/pieces/${product.id}`} className="block">
        <div className="relative aspect-[3/4] overflow-hidden bg-stone-100">
          ...
          {/* Tag */}
          {product.tag && (
            <span className="absolute top-3 left-3 text-[9px] uppercase tracking-[0.2em] bg-white/90 backdrop-blur-sm text-charcoal-900 px-2 py-1">
              {product.tag}
            </span>
          )}
      ...
      <div className="pt-3 pb-1">
        ...
        <p className="text-xs text-charcoal-400 mt-0.5">{product.designer}</p>
        <p className="text-sm text-charcoal-900 mt-2">{product.price}</p>
      </div>
    </div>
  );
}
