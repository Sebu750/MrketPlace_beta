    ...
    <section className="bg-cream-50 py-20 md:py-28">
      <div ref={ref} className="reveal max-w-[1440px] mx-auto px-6">
        ...
        <div className="mb-12">
          ...
          <h2 className="font-serif text-3xl md:text-4xl text-noir-900 font-medium">Trending Now</h2>
        ...
        <div className="columns-2 lg:columns-3 gap-4 md:gap-5 stagger-children space-y-4 md:space-y-5">
          {items.map((p) => (
            <Link key={p.id} to={`/pieces/${p.id}`}
              className="reveal group block break-inside-avoid relative overflow-hidden bg-parchment-100">
              ...
              {/* Hover tooltip */}
              <div className="absolute bottom-0 left-0 right-0 p-4 bg-gradient-to-t from-noir-900/70 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300">
